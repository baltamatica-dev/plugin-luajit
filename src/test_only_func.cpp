...
luajit::test::eval_str [测试函数] 测试从字符串执行 lua 脚本.

    luajit::test::eval_str(a, b) = a + b
    a,b 为 double, 对输入参数求和

示例：
    luajit::test::eval_str(1,2) == 3
...
luajit::test::eval_lua_file [测试函数] 测试从脚本加载并执行 lua 函数.

    luajit::test::eval_lua_file(a, b)
        输入 a,b 为 double;
        返回 double

默认示例：
    luajit::test::eval_lua_file(1,2) == 3

## dev note
测试用 lua 文件为: `luabundle/lua_func.lua`.
默认内容为:

    -- luajit::test::eval_lua_file 函数调用的测试文件
    function lua_add2 (a, b)
        local c = a + b - 1
